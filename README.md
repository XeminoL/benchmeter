...
**A noise floor.** A confidence interval describes the samples you collected and knows nothing about the machine drifting underneath them. During development this reported a 1.9% difference between two identical commands while the machine drifted 48%. The host is now profiled separately, and a difference has to clear its measured resolution as well.

The interleaving scheme is called RMIT in the literature. Added here: the packaging, and the second gate.
...
31 tests. The ones that matter are in `test_false_positives.py`, where the right answer is fixed in advance: samples from one distribution must never be called different, and a twofold difference must never be missed. The error rate is counted. A measurement tool that grades its own output is not evidence.

## What it won't do

It won't quiet your machine. Pinning to a core, raising priority and disabling the garbage collector were all tried; without admin rights the first two are refused, and the third changed nothing measurable. It profiles the machine and reports what it finds.
...
Every number here came from one Windows laptop. Yours will differ; `--self-proof` reproduces the experiment locally.
